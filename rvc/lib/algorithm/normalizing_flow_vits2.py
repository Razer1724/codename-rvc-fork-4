...
class ResidualCouplingTransformersLayer2(nn.Module):  # VITS2
    def __init__(
        ...
        # Since in VITS2 not a single transformer flow is spk conditioned
        # there's effectively no difference between picking RVC's and VITS2's encoder for pre_transformer
        self.pre_transformer = TransformerEncoderRVC(
            hidden_channels,
            hidden_channels,
            n_heads=2,
            n_layers=1,
            ...
            p_dropout=p_dropout,
            window_size=10, # VITS2 uses 4 everywhere, for RVC it's 10 so I decided to stick with the default.
        )
        self.enc = WaveNet(
            hidden_channels,
            kernel_size,
